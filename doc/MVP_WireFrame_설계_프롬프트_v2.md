# MVP WireFrame 설계 프롬프트 (범용 템플릿)

**목적**: 어떤 MVP IA 문서를 제공받더라도 전문적이고 효과적인 WireFrame을 설계하도록 AI를 가이드하는 프롬프트
...
다음 순서로 전체 WireFrame 패키지를 제공하세요:
...
2. wireframes/
   ├── 01_dashboard.md (React/JSX 코드)
   ├── 02_setup.md
   ├── 03_live.md
   └── ...

3. components/
   ├── common.md (공통 컴포넌트 스펙)
...
### 1.1 서비스 프로파일
- 서비스 유형: B2G EdTech (특수교육 체육 관리)
- 핵심 가치: 교사 부담 감소 + 학생 참여도 향상 + 객관적 평가
- MVP 범위: F1-F5 (실시간 동작 인식, 캐릭터 매칭, 운동 카운팅, AI 피드백, 교사 제어판)
- 기술적 제약: PWA, 20-30 FPS, WCAG AA 준수
...

## PHASE 2: WireFrame 설계 원칙

### 적용 프레임워크
1. Nielsen Norman - User Control (실수 복구 경로)
2. Material Design - 8dp Grid System  
3. WCAG 2.1 AA - 4.5:1 대비비
...
...
## PHASE 4: React/JSX WireFrame

[상세한 React 컴포넌트 구조 출력]
...
```
...
이 프롬프트는 다음 기준을 충족합니다:

✅ **범용성**: 모든 MVP IA 문서에 적용 가능  
✅ **전문성**: Nielsen Norman, Material Design 등 검증된 프레임워크 참고  
✅ **구체성**: 단계별 명확한 지침과 React/JSX 예시  
✅ **실용성**: 개발팀이 즉시 구현 가능한 산출물 (React 컴포넌트)  
✅ **확장성**: Post-MVP 고려사항 포함  
✅ **일관성**: React/JSX 형식으로 통일된 출력  

---

**작성 완료**: 이 프롬프트를 AI에게 IA 문서와 함께 제공하면, React/JSX 기반의 전문적인 MVP WireFrame을 설계할 수 있습니다.
